...
var isPaused=false;$(document).ready(function(){if(adTagUrl==""||ad_client!="vast"){return}pb = "view-player-";var player = videojs(pb + fk);var options = {adTagUrl: adTagUrl,adCancelTimeout: 5e3,playAdAlways: true,adsEnabled: true,vpaidFlashLoaderPath: ""};player.vastClient(options);player.on("vast.adStart",function(){count = typeof ad_skip != "undefined" ? parseInt(ad_skip) : 0;if (count > 0) {counter = setInterval(timer, 1e3);html = '<div class="vast-skip-button">Skip <span id="skipspan">in <span id="skiptime">' + count + "</span>...</span></div>";$(".video-js").append(html)}$(".vjs-ad-playing .vjs-resolution-button").css({display: "none"})});player.on("vast.adEnd",function(){$(".vjs-resolution-button").css({display: "block"});$(".vast-skip-button").detach()});player.on("pause",function(){if ($(".video-js").hasClass("vjs-ad-playing")) {isPaused = true;}});player.on("play",function(){if ($(".video-js").hasClass("vjs-ad-playing")) {isPaused = false;}});});function timer(){if(!isPaused){count=count-1;if(count<=0){clearInterval(counter);$("#skipspan").detach();$(".vast-skip-button").addClass("enabled");$(".vjs-ad-playing .vjs-progress-control").css({"pointer-events":"auto"});$(".vast-skip-button.enabled").click(function(){player.trigger("vast.adEnd");$(this).detach();$(".vjs-resolution-button").css({display:"block"})});return}document.getElementById("skiptime").innerHTML=count}}
