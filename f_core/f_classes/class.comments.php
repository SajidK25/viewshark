...
class VComments extends VView
{
    ...
    public function commLayout(string $type, int $vuid, $msg_arr = '', $like_arr = '')
    {
        ...
        /* remove response button on shorts */
        $ht_js .= '$(document).ready(function(){
			if($("#wrapper").hasClass("tpl_shorts")){
				...
				var w;if(h<=560){h=560;w=315}else{w=h*aspectRatio;}
				var odw = tw >= 1320 ? 360 : $(".video-js").width()-20;
        		odw = odw <= 748 ? (w <= 576 ? 210 : w) : odw;
        		var odp = tw <= 1320 ? "50%" : "15%";
				// alert(ct);
				$(".posted-comments").addClass("scroll " + th).css("height", ct);
				// $(".posted-comments").addClass("scroll " + th).css("height", h-143);
				$(".post-response-button").detach();
				$(".comm-cancel-action").css("margin-right", "10px");

				$(".posted-comments").on("mouseenter",function(e){
					//$(this).addClass("scroll " + th).css("overflow-y", "scroll");
					$(this).addClass("scroll " + th);
				}).on("mouseleave",function(e){
					//$(this).removeClass("scroll").removeClass(th).css("overflow-y", "hidden");
					//$(this).removeClass("scroll").removeClass(th);
				});

				$(".comments-close i").on("click",function(e){
					var ww=$(".video-js").width();
					$(".overlay-buttons").removeAttr("style");
					$(".overlay-data").css("width", odw);
					$(".overlay-data").removeAttr("style").css("width", odw);
					$(".overlay-comments").hide();
					$("#view-player").removeClass("with-comments");
				});

				$("#main-comments-actions .comm-cancel-action").on("click",function(e){
					// $(".posted-comments").css("height", h-143);
					var sh=!$("#view-player").hasClass("bottom-comments")?$(".video-js").height():360;
					var ct=sh - ($("#comment-load .file-views-nr").height() + $("#comment-load #comm-post-response").height() + $("#comment-load #comm-post-form").height() + 20);
        ...
        $ht_js .= 'e.preventDefault();var t=$(this);var v=t.attr("href").replace("#", "");$("#comm-sort").val(v);var frm=$("#comm-post-form").serialize();';
        ...
        if ($islogged) {
            ...
            // $ht_js .= 'var ct=$(".video-js").height() - ($("#comment-load .file-views-nr").height() + $("#comment-load #comm-post-response").outerHeight() + $("#comment-load #comm-post-form").height() + 20);setTimeout(() => {$(".posted-comments-OFFFFFF").css("height", ct-13);}, "100");';
            $ht_js .= '});}});';
			...
			if(window.innerWidth<500){
				...
				$(".posted-comments > .comments_activity > .response_holder #comment-emotes"+f1+" em-emoji-picker").css("min-width", window.innerWidth-20).css("left", (window.innerWidth-(window.innerWidth+21)));
			} else {
			...
			if(window.innerWidth<500){
				...
				$(".posted-comments > .comments_activity > .response_holder #ecomment-emotes"+f1+" em-emoji-picker").css("min-width", window.innerWidth-20).css("left", (window.innerWidth-(window.innerWidth+21)));
			} else {
        ...
        $comm_btn  = ($cfg['file_comments'] == 1 ? '<span class="place-right showSingle-lb-off showComments-eye mt-15" target="comments"><i class="icon-eye-blocked" rel="tooltip" title="' . $language['subnav.entry.comments'] . '"></i></span>' : null);
        ...
        $html = '
				<div class="page_holder comm-wrapper" id="' . $type . '-comm-wrapper">
					<div class="file-views-nr">
					    ...
					    <div class="comments-close"><i class="icon-times"></i></div>
					    ' . $comm_btn . '
					</div>
					...
					<div id="ntm" style="display: block;">
						...
						<div class="comments_actions main_comments_actions" id="main-comments-actions" style="display:none">
							...
							<a href="' . $cfg['main_url'] . '/' . VHref::getKey("respond") . '?' . $type[0] . '=' . $f_key . '" class="comm-cancel-action link cancel-trigger post-response-button place-left" rel="nofollow">' . str_replace('##TYPE##', $language["frontend.global." . $type[0]], $language['view.files.resp.post']) . '</a>
							' : null) . '
						</div>
						</div>
					...
					' . VGenerate::simpleDivWrap('posted-comments', 'list-' . $type . '-comments', self::listFileComments($type, $vuid)) . '
...
' . ($_SESSION['USER_ID'] > 0 ? '
			...
			textarea.addEventListener("input", () => {
				...
				textarea.style.height = (textarea.scrollHeight+1) + "px";
				if ($("#wrapper").hasClass("tpl_shorts")) {
					var pch=$("#comm-post-form").height() + 92;
					var h=$("body").height()-100;
					// $(".posted-comments").css("height",(h-pch));
					var sh=!$("#view-player").hasClass("bottom-comments")?$(".video-js").height():360;
					var ct=sh - ($("#comment-load .file-views-nr").height() + $("#comment-load #comm-post-response").height() + $("#comment-load #comm-post-form").height() + 20);
					$(".posted-comments").css("height", ct);
				}
...
' : null) . '
    ...
    public function listFileComments(string $type, int $vuid, $getReplies = false)
    {
        ...
        if (!$getReplies and $page_nr == 1) {
            $comm_sql = sprintf("SELECT
                    ...
                    B.`affiliate_badge`, B.`usr_dname`, B.`ch_title`, C.`comment_spam`, C.`comment_votes`
                    FROM `db_%scomments` A
                    JOIN `db_accountuser` B ON A.`c_usr_id`=B.`usr_id`
                    JOIN `db_%sfiles` C ON A.`file_key`=C.`file_key`
                    WHERE %s A.`file_key`='%s' %s %s
                    AND A.`c_pinned`='1'
                    AND A.`c_active`='1'
                    ORDER BY `%s` DESC;
        ", $type, $type, (isset($u_blocked[0]) ? sprintf("`c_usr_id` NOT IN(%s) AND", implode(",", $u_blocked)) : null), $file_key, $sort_approved, $sort_spam, $sort_sql);

            $comm_db  = self::$db_cache ? $db->CacheExecute($cfg['cache_view_comments'], $comm_sql) : $db->execute($comm_sql);
            $comm_rs1 = $comm_db->getrows();

            if (isset($comm_rs1[0])) {
                foreach ($comm_rs1 as $key => $val) {
                    $ex_id[] = (int) $val['c_id'];
                }
            }
        ...
        $comm_sql = sprintf("SELECT
					...
					JOIN `db_accountuser` B ON A.`c_usr_id`=B.`usr_id`
					JOIN `db_%sfiles` C ON A.`file_key`=C.`file_key`
					WHERE %s %s %s AND A.`file_key`='%s' %s %s
					AND A.`c_active`='1'
					ORDER BY
					CASE WHEN A.`c_pinned`='1' THEN 0 ELSE 1 END, `%s` DESC
					LIMIT %s, %s;
		", $type, $type, (isset($ex_id[0]) ? sprintf("A.`c_id` NOT IN (%s) AND ", implode(",", $ex_id)) : null), (isset($u_blocked[0]) ? sprintf("`c_usr_id` NOT IN(%s) AND", implode(",", $u_blocked)) : null), $reply_s, $file_key, $sort_approved, $sort_spam, $sort_sql, $s_from, $cpp);
        ...
        if (!$getReplies and $page_nr == 1) {
            foreach ($comm_rs1 as $v => $val) {
                array_unshift($comm_rs, $val);
                // exit;
            }
        }
        ...
        if ($total_nr > 0) {
            ...
            foreach ($comm_rs as $key => $val) {
                ...
                $html .= '	' . ((($c_replyto > 0 and $sort_by != 'not-approved' and $sort_by != 'spam-reports' and $sort_by != 'new-replies' and $getReplies) ? '<div class="response_holder" style="display:none">' : null)) . '
					<div id="' . $c_key . '" class="comments_activity' . (($c_replyto > 0 and $sort_by != 'not-approved' and $sort_by != 'spam-reports' and $sort_by != 'new-replies' and $getReplies) ? ' response' : null) . $_class_ht . '" rel-resp="' . $c_replyto . '">
						<div class="usr_img">
					...
					' . ((($c_replyto > 0 and $sort_by != 'not-approved' and $sort_by != 'spam-reports' and $sort_by != 'new-replies' and $getReplies) ? '</div>' : null)) . '
					';
    ...
    public function calculateString($mathString)
    {
        ...
        // allow only digits, operators, spaces, parentheses
        $expr = preg_replace('/[^0-9\+\-\*\/\(\) ]/', '', $expr);
        // handle unary minus: prefix 0 where needed
        if ($expr !== '' && $expr[0] === '-') {
            $expr = '0' . $expr;
        }
    ...
    private static function evaluateExpression($expr)
    {
        ...
        while ($i < $len) {
            ...
            // operator
            if ($ch === '+' || $ch === '-' || $ch === '*' || $ch === '/') {
                while (!empty($ops) && $precedence(end($ops)) >= $precedence($ch)) {
                    $op = array_pop($ops);
                    $apply($nums, $op);
                }
                $ops[] = $ch; $i++; continue;
            }
            // unknown char (shouldn't happen after sanitize)
            $i++;
        }
        while (!empty($ops)) {
            $op = array_pop($ops);
    ...
    public function postComment()
    {
        ...
        if (is_array($cfg['maxmind_country']) and in_array($country, $cfg['maxmind_country'])) {
            return VComments::commLayout($type, $upage_id, array($language['notif.success.request'], ''));
        }
        ...
        if ($error_message == '') {
            ...
            /* log comments */
            $log = ($cfg['activity_logging'] == 1 and $action = new VActivity($_SESSION['USER_ID'], $upage_id)) ? $action->addTo('log_filecomment', $type . ':' . $file_key . ':' . $c_key) : null;
            /* mailing */
        ...
        return VComments::commLayout((string) $type, (int) $upage_id, array($notice_message, $error_message));
    }
            ...
            case "comm-block":
                ...
                if ($comm_uid == (int) $_SESSION['USER_ID']) {
                    ...
                    if ($c_id = $db_c->fields['c_id']) {
                        ...
                        if ($db_id > 0) {
                            return VComments::commLayout((string) $type, (int) $upage_id, array('', ''));
                        } else {
                            return;
                        }
                    } else {
    ...
    public function browseComment()
    {
        ...
        return $page ? VComments::listFileComments($type, $upage_id) : VComments::commLayout($type, $upage_id);
    }
