	...
	<div id="overview-userinfo">
		...
		<article class="no-display">
        	...
        	<div id="account-menu-entry13" class="swiper-slide{if $smarty.get.rg eq "" and $smarty.get.rp eq ""} swiper-slide-current{/if}" rel-m="{href_entry key="tokens"}"><a class="{if $smarty.get.rg eq "" and $smarty.get.rp eq ""}dcjq-parent active{/if}" href="{$main_url}/{href_entry key="tokens"}"><span><i class="icon-user"></i> {lang_entry key="account.entry.overview"}</span></a></div>
        	{if $smarty.session.USER_PARTNER eq 1}
        	<div id="account-menu-entry9" class="swiper-slide{if $smarty.get.rg ne ""} swiper-slide-current{/if}" rel-m="{href_entry key="tokens"}"><a class="{if $smarty.get.rg ne ""}dcjq-parent active{/if}" href="{$main_url}/{href_entry key="tokens"}?rg={$smarty.session.USER_KEY|md5}"><span><i class="icon-bars"></i> {lang_entry key="account.entry.tokens.stats"}</span></a></div>
        	<div id="account-menu-entry12" class="swiper-slide{if $smarty.get.rp ne ""} swiper-slide-current{/if}" rel-m="{href_entry key="tokens"}"><a class="{if $smarty.get.rp ne ""}dcjq-parent active{/if}" href="{$main_url}/{href_entry key="tokens"}?rp={$smarty.session.USER_KEY|md5}"><span><i class="icon-paypal"></i> {lang_entry key="account.entry.tokens.pay"}</span></a></div>
        	{/if}
    				</div>
    			</div>
    		</div>
