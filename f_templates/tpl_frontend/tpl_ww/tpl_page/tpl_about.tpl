<div class="lb-margins">
  <article>
    <h3 class="content-title"><i class="icon-rss"></i>About Page</h3>
    <div class="line"></div>
  </article>
  <div>
    <h1 class="list-h2">About Page</h1>
	<h2 class="sp-h1">Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Sed elit.</h2>
	<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Sed elit. Nulla sem risus, vestibulum in, volutpat eget,                            dapibus  ac, lectus. Curabitur dolor sapien, hendrerit non, suscipit bibendum, auctor ac, arcu. Vestibulum dapibus.                            Sed pede lacus, pretium in, condimentum sit amet, mollis dapibus, magna. Ut bibendum dolor nec augue. Ut tempus                             luctus metus. Sed a velit. Pellentesque at libero elementum ante condimentum sollicitudin. Pellentesque lorem ipsum,                             semper quis, interdum et, sollicitudin eu, purus. Vivamus fringilla ipsum vel orci. Phasellus vitae massa at massa                             pulvinar pellentesque. Fusce  tincidunt libero vitae odio. Donec malesuada diam nec mi.                     </p>
	<div class="clearfix"></div>
	<div class="sp-wrap bottom-border">
	    <div class="sp-headings">
		<ul>
		    <li><h1>Heading 1</h1></li>
		    <li><h2>Heading 2</h2></li>
		    <li><h3>Heading 3</h3></li>
		    <li><h4>Heading 4</h4></li>
		    <li><h5>Heading 5</h5></li>
		    <li><h6>Heading 6</h6></li>
		</ul>
	    </div>
      <br>
      <h2>Heading 2</h2>
		<p>Vivamus tortor nisl, lobortis in, faucibus et, tempus at, dui. Nunc risus. Proin scelerisque augue. Nam                                ullamcorper. Phasellus id massa. Pellentesque nisl. Pellentesque habitant morbi tristique senectus et netus et                                malesuada fames ac turpis egestas.                         </p>
		<h2>Heading 2</h2>
		<p>Maecenas ullamcorper, odio vel tempus egestas, dui orci faucibus orci, sit amet aliquet lectus dolor et quam.          
          <p><u>Vivamus tortor</u> nisl, lobortis in, faucibus et, tempus at, dui. Nunc risus. Proin scelerisque augue. Nam                        ullamcorper. Phasellus id massa. Pellentesque nisl. Pellentesque habitant morbi tristique senectus et netus et malesuada                        fames ac turpis egestas. Nunc augue. Aenean sed justo non leo vehicula laoreet. Praesent ipsum libero, auctor ac, tempus                        nec, tempor nec, justo.                    </p>
				<p>Vivamus tortor nisl, lobortis in, faucibus et, tempus at, dui.    </p>
          <ul>
            <li><i class="iconBe-chevron-right"></i> Lorem ipsum dolor sit amet, consectetuer.</li>
		    <li><i class="iconBe-chevron-right"></i> Morbi luctus. Duis lobortis. Nulla nec velit.</li>
		    <li><i class="iconBe-chevron-right"></i> Mauris pulvinar erat non massa. Suspendisse.</li>
		    <li><i class="iconBe-chevron-right"></i> Turpis, porta nec, tempus vitae, iaculis semper.</li>
		    <li><i class="iconBe-chevron-right"></i> Cras vel libero id lectus rhoncus porta. Sus</li>
		    <li><i class="iconBe-chevron-right"></i> Lorem ipsum dolor sit amet, consectetuer </li>
		    <li><i class="iconBe-chevron-right"></i> Morbi luctus. Duis lobortis. Nulla nec velit.</li>
		    <li><i class="iconBe-chevron-right"></i> Mauris pulvinar erat non massa. Suspendisse</li>
		    <li><i class="iconBe-chevron-right"></i> Turpis, porta nec, tempus vitae, iaculis semper.</li>
		    <li><i class="iconBe-chevron-right"></i> Cras vel libero id lectus rhoncus porta. Suspendisse</li>
		</ul>
  </div>
</div>